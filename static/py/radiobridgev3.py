...
class Decoder(object):
    ...
    def decodePayload(self, msg_dict, payload_bytes):
        ...
        elif event == DEVICE_INFO_EVENT:
            decoded = self.handle_DEVICE_INFO(payload_bytes)
        else:
            return None
    ...
    def handle_SUPERVISORY(self, payload_bytes):
        ...
        # BT - 19 01 01 02 30 01 00 00 00 00 04
        # 19: protocol and counter
        # 01: event
        # 01: sensor state - depreciated.
        # 30: Battery level - 30 read as 3.0v
        # 01 00 00 00: Current sensor state.
        # 00 04: Event Accoumulation Count
    ...
    # ==================   DEVICE INFO EVENT  ====================
    def handle_DEVICE_INFO(self, payload_bytes):
         
        decoded = {}
        # BT - Specify what event it is.
        decoded["event"] = "device_info"

        # BT - hex: b'\x13\xfa\x11\t\x01\x93\x00\x01\x01\x00\x00'
        # BT - rb_data_decoded: <class 'NoneType'>
	    #"data_decoded":{"DEVICE_INFO":{"downlinkBytes":"01 93 00 01 01 00 00","event":"Device Info Message 1 of 1","packetType":null},"counter":19,"protocol":"1","type":"DEVICE_INFO"}
	    #{"BatteryLow":false,"ErrorWithLastDownlink":false,"TamperState":false,"TamperSinceLastReset":false,"threshold":"X-axis over threshold,,Z-axis over threshold,Settling window time expired","battery":"3.0V","accumulationCount":0}
        device_info = payload_bytes[2]

        # Get the first nibble (shift 4 bits to the right)
        index = (device_info >> 4) & 0x0F
        
        # Get the second nibble (lower 4 bits)
        total = device_info & 0x0F

        # BT - 
        decoded['message'] = f'{index} of {total}'
        
        # Convert payload bytes to two-digit hex values and join them
        downlink_bytes = ' '.join([f'{byte:02X}' for byte in payload_bytes[3:10]])

        decoded['downlinkBytes'] = downlink_bytes
